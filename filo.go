	...
	if todosfilosofos[recorreF].Izquierda==1 && todosfilosofos[recorreF].Derecha==0 {
		if recorreF==(n-1) {
			if Tenedores[recorreF].tomado==0 {
				...
				fmt.Println("\ttenedor ",(recorreF+1)," levantado por Filosofo ",(recorreF+1),"\n")
			}else{
			...
			if Tenedores[recorreF].tomado == 0{
				...
				fmt.Println("\tFilosofo ",(IDdublicado+1)," esta esperando para tenedor ",(recorreF+1),"\n")
			}
		}
	}
	if todosfilosofos[recorreF].Izquierda==0 {
		if recorreF==(n-1) {
			if Tenedores[recorreF-1].tomado == 0 {
				...
				fmt.Println("\tFilosofo ",(recorreF+1)," esta esperando for tenedor ",recorreF,"\n")
			}
		}else{
			if Tenedores[recorreF].tomado == 0{
				Tenedores[recorreF].tomado = 1
...
func main(){
	...
	for filosofosSaciados<n{
		...
		fmt.Println("\t hasta ahora el numero de filosofos que Termino la cena es ",filosofosSaciados,"\n\n")
	}
